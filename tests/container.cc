...
TEST(iterator, stdarray) {
  ...
  EXPECT_EQ(serialize(a), "[ 1, 2, 3 ]");
}

TEST(iterator, list) {
  std::list<int> l1 {  };
  EXPECT_EQ(serialize(l1), "[  ]");
  std::list<int> l2 = { 1, 2 };
  EXPECT_EQ(serialize(l2), "[ 1, 2 ]");
  std::list<int> l3 = { 1, 2, 3 };
  EXPECT_EQ(serialize(l3), "[ 1, 2, 3 ]");
}
...
TEST(pair, intsring) {
  ...
  EXPECT_EQ(serialize(std::make_pair(1, "one"s)), R"([ 1, "one" ])");
  EXPECT_EQ(serialize(std::make_pair(2, "two")), R"([ 2, "two" ])");
}

TEST(tuple, tuple) {
  using namespace std::string_literals;
  EXPECT_EQ(serialize(std::make_tuple()), R"([  ])");
  EXPECT_EQ(serialize(std::make_tuple("A")), R"([ "A" ])");
  EXPECT_EQ(serialize(std::make_tuple("A", "B"s, 1337)), R"([ "A", "B", 1337 ])");
}
